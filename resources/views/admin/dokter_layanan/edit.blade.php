  ...
  <div class="container-fluid">
    ...
    <div class="card shadow mb-4">
      <div class="card-header py-3">
        <h5 class="font-weight-bold text-primary">Edit Dokter Layanan</h5>
      </div>
      <div class="card-body">
        <form method="POST" action="{{ route('dokter_layanan.update', $dokterLayanan->id) }}">
          @csrf
          @method('PUT')
          ...
          <div class="form-group">
            ...
            <select class="form-control @error('dokter_id') is-invalid @enderror" name="dokter_id" id="dokter_id">
              ...
              @foreach($dokters as $dokter)
                <option value="{{ $dokter->id }}" {{ (old('dokter_id', $dokterLayanan->dokter_id) == $dokter->id) ? 'selected' : '' }}>{{ $dokter->nama }}</option>
              @endforeach
            </select>
          ...
          <div class="form-group">
            ...
            <select class="form-control @error('layanan_id') is-invalid @enderror" name="layanan_id" id="layanan_id">
              ...
              @foreach($layanans as $layanan)
                <option value="{{ $layanan->id }}" {{ (old('layanan_id', $dokterLayanan->layanan_id) == $layanan->id) ? 'selected' : '' }}>{{ $layanan->nama }}</option>
              @endforeach
            </select>
          ...
          <div class="form-group">
            ...
            <select class="form-control @error('jadwal_id') is-invalid @enderror" name="jadwal_id" id="jadwal_id">
              ...
              @foreach($jadwals as $jadwal)
                <option value="{{ $jadwal->id }}" {{ (old('jadwal_id', $dokterLayanan->jadwal_id) == $jadwal->id) ? 'selected' : '' }}>
